...
const Minorcourses = () => { 
    ...
    // React.useCallback.
    const toggleModal = React.useCallback((key) => () => {
        setIsOpen(true);
        console.log(`Post id: ${key}`);
    }, []);
   
    const toggleModalclose = React.useCallback((key) => () => {
        setIsOpen(false);
    }, []);
        ...
        {posts.map((post) => (
          <div className="col-md-3" key={post.id}>
              ...
              <div class="content">
				...
				<ul class="sci">
					<li>
						<span  onClick={toggleModal(post.id)} to="/">Read More</span >
					</li>
				</ul>
                </div>
              <Modal
